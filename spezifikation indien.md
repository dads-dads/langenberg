...
**Datenstand:** `indien_karte.json` v0.95 · integriert in index.html v7.15
...
- Kartenbild: `indien_karte.jpg` (2400×3588 px, `board_h: 1495`), Ticketbild: `auftrag_indien.jpg` (700×1120, Niederlande-Stil: NL-Rahmen als Vorlage, ausgeblichener Spielplan 661×992 bei 20/118 eingeklebt)
- Ticket-Minikarte: `tk_vb` 700×1120, hochkant, Einzelziel-Layout; `tk`-Box {20, 118, 661, 992}, tkx/tky daraus berechnet (Kontrollbild: alle 39 Städte deckungsgleich)
...
- **v0.94** – Integration in `index.html` (App v7.14): Mandala-Bonus in Endabrechnung, Showdown und Abrechnungstabelle implementiert (streckendisjunkter Doppelweg-Check per Einheitsfluss); Indian Express über `laengste_strecke_bonus: 10`; Fähren nutzen die vorhandene Lok-Pflicht-Logik.
- **v0.95** – Auftragskarte `auftrag_indien.jpg` im Niederlande-Stil neu erzeugt (Ausbleich-Faktor 0.52 auf NL-Helligkeit kalibriert); `tk_vb` auf 700×1120, tkx/tky neu berechnet. App v7.15.

## Offene Punkte

1. Manmad–Poona grau 1 ist noch interpoliert (Mittelpunkt) – ggf. im Editor nachziehen
2. `indien_karte.jpg` und `auftrag_indien.jpg` (neu erzeugt) müssen im GitHub-Pages-Repo liegen
