...
- Spielsteine (App v7.44): `waggon()` und `schiffStein()` zeichnen die Steine als SVG aus der Spielerfarbe (`steinFarben()` mischt Licht-, Schatten-, Rand- und Radfarbe). Zugstein = Kesselwagen mit drei Rädern (25 % der Steinhöhe), Schiffsstein = Dampfer mit Aufbau, Schornstein und Mast; der Bug zeigt immer zur Zielstadt. Die Datei `steine.png` wird nicht mehr benötigt. Gilt für alle Editionen; nur der Schiffsstein ist auf Weltreise-Schiffsstrecken beschränkt.
- Offen für den Praxistest: Layout der 6 + 2 Karten in der rechten Auslage auf kleinen Displays.
